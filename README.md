...
do the actual work — so you stop being the bottleneck who keeps the whole portfolio
in your head.
...
SQLite ledger. Kill the coordinator and start a fresh one anytime — there is no
...
pwc/                      # this repo — the SOURCE (develop & commit here)
  schema.sql              # the ledger schema
  scripts/                # shared deterministic mechanism ("the hands")
    ledger.py             #   single read/write path to the ledger (CLI, JSON I/O)
    spawn.py              #   spawn a worker in an iTerm2 window
    liveness.py           #   is a worker session still running? (pgrep)
    pwc_db.py, _common.py #   shared connection / discovery helpers
  skills/                 # the coordinator's brain (SKILL.md each)
    brief/  next/  dispatch/  pwc-report/
  install.sh              # symlink skills into a workspace + init its ledger
```

Runtime state lives **in each workspace**, never here: `<workspace>/.pwc/ledger.db`.

## Install

PWC installs per-workspace by symlinking its skills into the workspace's
`.claude/skills/` and initializing a ledger — the same pattern as `team-skills`.

```bash
./install.sh ~/work/acme     # or omit the arg for that default
```

This creates `<workspace>/.claude/skills/{brief,next,dispatch,pwc-report}` (symlinks
back here) and `<workspace>/.pwc/ledger.db`. Because the skills are symlinks,
`git pull` in this repo upgrades every installed workspace at once.

### Prerequisites

- **Python 3** (stdlib `sqlite3` — no packages needed for the ledger).
...
- `pgrep` (standard on macOS) — for liveness checks.

## Use

Start a Claude Code session **in the workspace** and:

- **`/brief`** — the whole-portfolio briefing. Run it anytime (morning to orient,
  midday to check in, close to wrap up). Reconciles against your sources, sweeps for
  dead workers and stale tasks, notices inbound, rolls up the day, and presents a
  prioritized view.
- **`/next`** — suggests what to start or resume next. Suggests only; never acts
  without your confirmation.
- **dispatch** — turns a task into action: spawns a worker (default) or handles it
...
```bash
python3 scripts/ledger.py --workspace <ws> summary
python3 scripts/ledger.py --workspace <ws> detail --task t_0007
```
...
cases): **reconciliation conflict rules** and **inbound new-vs-update matching**.
