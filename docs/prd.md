...
- **History lives in the schema, not in git.** A SQLite file doesn't diff, so
  the longitudinal record (what `/pwc-show-work`'s recap and the journal draw on)
...
2. **Briefing (`/pwc-show-work`).** The single interactive surface — run it anytime
...
3. **Next-action decision (`/pwc-pick-work`).** Given current task database state, suggest
...
4. **Worker dispatch (long-running tasks).** Launch a Claude Code worker for a
   ...
   **Dispatch covers resumption — there is no separate resume command.** Picking
   ...
   changed externally since I last touched this" — comes from `/pwc-show-work`'s per-task
...
6. **Worker status tracking.** Workers update the task database as they hit meaningful
   ...
   surfaces naturally whenever a later `/pwc-show-work` re-checks that task's external
   state.
   ...
   **worker-status check.** Self-reporting only covers workers healthy enough to
   ...
   This stays within the on-demand model: worker-status check is evaluated at `/pwc-show-work`,
...
- Background polling — sources are checked on-demand via `/pwc-show-work`.
